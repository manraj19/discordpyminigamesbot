...
async def help(ctx):
    embed = discord.Embed(title="Help", description="Use b;help <command> for more info on a command.", color=0xf1c40f)
    ...
    embed.add_field(name="Utility", value="`leaderboard`, `define`, `urbandictionary`, `botinfo`", inline=False)
...
async def helpmessage(interaction: discord.Interaction):
    embed = discord.Embed(title="Help", description="Use ;help <command> for more info on a command.", color=0xf1c40f)
...
async def on_app_command_error(interaction: discord.Interaction, error: app_commands.AppCommandError):
    if isinstance(error, app_commands.CommandNotFound):
        await interaction.response.send_message("That command does not exist. Please use `/help` to see the list of commands.", ephemeral=True)
...
async def dino(interaction: discord.Interaction):
    ...
    await interaction.response.send_message("Starting Dino Run game...")

    while True:
        obstacle = random.choice(obstacles)
        end_time = discord.utils.utcnow().timestamp() + response_time
        if obstacle == "cactus":
            await interaction.followup.send(f"You're running towards a cactus. You have to respond <t:{int(end_time)}:R> (jump/duck)")
            try:
                response = await bot.wait_for('message', check=lambda m: m.author == interaction.user and m.content.lower() in ['jump', 'duck'], timeout=response_time)
                if response.content.lower() == 'jump':
                    score += 1
                    if score >= 30:
                        response_time = 1.8  # Set response time to 1.8 second if score reaches 30
            ...
            await interaction.followup.send(f"A bird is flying towards you. You have to respond <t:{int(end_time)}:R> (jump/duck)")
            try:
                response = await bot.wait_for('message', check=lambda m: m.author == interaction.user and m.content.lower() in ['jump', 'duck'], timeout=response_time)
                if response.content.lower() == 'duck':
                    score += 1
    ...
    await interaction.followup.send(f"Game over! Your final score is {score}.")
    update_score(interaction.user.id, str(interaction.user), score, 'dino')
...
class RPSView(View):
    ...
    def determine_winner(self, player_choice, opponent_choice):
        ...
        if winning_conditions[player_choice] == opponent_choice:
            update_score(self.player.id, str(self.player), 1, 'rockpaperscissors')  # Add 1 point to the player's score
            return f"{self.player.mention} wins!"
        else:
            update_score(self.opponent.id, str(self.opponent), 1, 'rockpaperscissors')  # Add 1 point to the opponent's score
...
async def rockpaperscissors(ctx, opponent: discord.Member):
    ...
    await ctx.send(f"{ctx.author.mention}, what is your choice?", view=view)
    await ctx.send(f"{opponent.mention}, what is your choice?", view=view)

    try:
        await bot.wait_for('interaction', check=lambda i: i.user in [ctx.author, opponent] and i.data['custom_id'] in ['Rock', 'Paper', 'Scissors'], timeout=10.0)
    except asyncio.TimeoutError:
        await ctx.send("Time's up! The game has ended due to inactivity.")
        view.stop()
...
async def rockpaperscissors(interaction: discord.Interaction, opponent: discord.Member):
    ...
    await interaction.response.send_message(f"{interaction.user.mention}, what is your choice?", view=view)
    await interaction.followup.send(f"{opponent.mention}, what is your choice?", view=view)

    try:
        await bot.wait_for('interaction', check=lambda i: i.user in [interaction.user, opponent] and i.data['custom_id'] in ['Rock', 'Paper', 'Scissors'], timeout=10.0)
    except asyncio.TimeoutError:
        await interaction.followup.send("Time's up! The game has ended due to inactivity.")
        view.stop()
...
async def tictactoe(ctx, opponent: discord.Member):
    ...
    message = await ctx.send(f"It's {ctx.author.mention}'s turn!", view=view)
...
async def tictactoe(interaction: discord.Interaction, opponent: discord.Member):
    ...
    message = await interaction.followup.send(f"It's {interaction.user.mention}'s turn!", view=view)
    view.message = message
...
async def wordle(interaction: discord.Interaction):
    ...
    def check(m):
        return m.author == interaction.user and m.channel == interaction.channel and len(m.content) == 5

    await interaction.response.send_message(f"Guess the word! {display}")

    while attempts > 0 and not guessed:
        try:
            guess = await bot.wait_for('message', check=check, timeout=60.0)
        except asyncio.TimeoutError:
            await interaction.followup.send(f"Time's up! You didn't guess the word in time. The word was: {word}")
            return
        ...
        if guess_content == word:
            ...
            update_score(interaction.user.id, str(interaction.user), 1, 'wordle')  # Add 1 point on win
            return
        ...
        attempts -= 1
        await interaction.followup.send(f"Guess: {guess.content}\nFeedback: {feedback}\nAttempts left: {attempts}")

    if not guessed:
        await interaction.followup.send(f"Sorry, you've run out of attempts! The word was: {word}")
...
@bot.tree.command(name="mathematics", description="Solve mathematical problems")
@app_commands.describe(quiz_type="The type of quiz: addition, subtraction, multiplication, or division")
@app_commands.checks.cooldown(1, 10, key=lambda i:(i.user.id))
async def mathematics(interaction: discord.Interaction, quiz_type: str):
    if quiz_type not in ['addition', 'subtraction', 'multiplication', 'division']:
        await interaction.response.send_message("Invalid quiz type! Please choose from addition, subtraction, multiplication, or division.", ephemeral=True)
        return
    ...
    await interaction.response.send_message(f"Starting {quiz_type} quiz!")
    ...
    while True:
        ...
        end_time = discord.utils.utcnow().timestamp() + response_time
        await interaction.followup.send(f"Solve: {question}\nYou have to answer <t:{int(end_time)}:R>.")
        ...
        try:
            ...
            if quiz_type == 'division':
                try:
                    if abs(float(guess.content) - answer) < 0.01:  # Allow a small margin of error
                        ...
                        await interaction.followup.send(f"Wrong! The correct answer was {answer}. Your score is {score}")
                        break
                except ValueError:
                    await interaction.followup.send(f"Invalid input! The correct answer was {answer}. Your score is {score}")
                    break
            else:
                if int(guess.content) == answer:
                    score += 1
            ...
            await interaction.followup.send(f"Time's up! The correct answer was {answer}. Your score is {score}")
            break
...
async def flagle(ctx):
    ...
    'YE': 'Yemen', 'ZM': 'Zambia', 'ZW': 'Zimbabwe'
        ...
        end_time = discord.utils.utcnow().timestamp() + response_time
        await ctx.send(f"Guess the country for this flag: [Flag]({flag_url})\nYou have to answer <t:{int(end_time)}:R>.")
        ...
        try:
            ...
            if guess.content.strip().lower() == country_name.lower():
                ...
                await ctx.send(f"Wrong! The correct answer was {country_name}. Your score is {score}.")
                break
        except asyncio.TimeoutError:
            await ctx.send(f"Time's up! The correct answer was {country_name}. Your score is {score}.")
            break
...
async def flagle(interaction: discord.Interaction):
    ...
    'YE': 'Yemen', 'ZM': 'Zambia', 'ZW': 'Zimbabwe'
        ...
        end_time = discord.utils.utcnow().timestamp() + response_time
        if score == 0:
            await interaction.response.send_message(f"Guess the country for this flag: [Flag]({flag_url})\nYou have to answer <t:{int(end_time)}:R>.")
        else:
            await interaction.followup.send(f"Guess the country for this flag: [Flag]({flag_url})\nYou have to answer <t:{int(end_time)}:R>.")
        def check(m):
            return m.author == interaction.user and m.channel == interaction.channel
        ...
        try:
            ...
            if guess.content.strip().lower() == country_name.lower():
                ...
                await interaction.followup.send(f"Wrong! The correct answer was {country_name}. Your score is {score}.")
                break
        except asyncio.TimeoutError:
            await interaction.followup.send(f"Time's up! The correct answer was {country_name}. Your score is {score}.")
            break
...
CRIC_API_KEY = os.environ.get("CRICAPI_KEY", "")  # CricAPI key
...
async def playcricket(interaction: discord.Interaction):
    ...
    await interaction.response.send_message("First Half: You are batting. Type a number (1, 2, 3, 4, 6) to play your shot.")

    # First Half: User is batting
    while True:
        try:
            user_message = await bot.wait_for('message', check=lambda m: m.author == interaction.user and m.content.isdigit() and int(m.content) in options, timeout=30.0)
            user_choice = int(user_message.content)
            bot_choice = random.choice(options)
            await interaction.followup.send(f"You chose {user_choice}, Bot chose {bot_choice}")

            if user_choice == bot_choice:
                await interaction.followup.send(f"Out! Your score: {user_score}")
                break
            else:
                user_score += user_choice
        except asyncio.TimeoutError:
    ...
    await interaction.followup.send("Second Half: Bot is batting. Type a number (1, 2, 3, 4, 6) to bowl.")
    ...
    while bot_score <= user_score:
        try:
            ...
            if user_choice == bot_choice:
                await interaction.followup.send(f"Out! Bot's score: {bot_score}")
                break
            else:
                bot_score += bot_choice
        except asyncio.TimeoutError:
...
async def botinfo(ctx):
    ...
    embed.set_thumbnail(url=bot.user.avatar)
    embed.add_field(name="Number of Servers", value=num_servers, inline=True)
...
class TriviaView(View):
    def __init__(self, ctx, questions):
        super().__init__(timeout=15)  # 15-second timeout per question
        self.ctx = ctx
        ...
        self.correct_answer = ""
        self.message = None
        self.bot = ctx.bot  # Access the bot instance from the initial context
        random.shuffle(self.questions)

    async def start_game(self):
        await self.next_question(self.ctx)

    async def next_question(self, ctx_or_interaction):
        if not self.questions:
            await ctx_or_interaction.send(f"Game over! Your final score is: {self.score}")
            self.stop()
            return
        ...
        options = self.current_question['incorrectAnswers'] + [self.correct_answer]
        random.shuffle(options)

        # Update buttons with new options
        for i, option in enumerate(options):
            self.children[i].label = option
            self.children[i].custom_id = option  # Set custom ID to match the label

        end_time = discord.utils.utcnow().timestamp() + 15
        ...
        embed.add_field(name="Time Left", value=f"<t:{int(end_time)}:R>")  # Add countdown timer

        if self.message:
            await self.message.edit(embed=embed, view=self)
        else:
            if isinstance(ctx_or_interaction, discord.Interaction):
                self.message = await ctx_or_interaction.followup.send(embed=embed, view=self)
            else:
                self.message = await ctx_or_interaction.send(embed=embed, view=self)

        # Start the countdown and check if the user answers in time
        self.bot.loop.create_task(self.start_timer(ctx_or_interaction))

    async def start_timer(self, ctx_or_interaction):
        await asyncio.sleep(15)
        # Check if the user failed to answer in time
        if not self.current_question:
            return  # If there are no more questions, the game is over

        await self.end_game(ctx_or_interaction, correct=False)
    ...
    async def button_callback(self, interaction: discord.Interaction):
        ...
        if chosen_answer == self.correct_answer:
            ...
            await self.end_game(interaction, correct=False)

    async def end_game(self, ctx_or_interaction, correct):
        result_msg = f"The correct answer was: **{self.correct_answer}**. Your score: {self.score}"
        ...
        self.stop()  # End the game view

@bot.command(aliases=['quiz'])
@commands.cooldown(1, 10, commands.BucketType.user)
async def trivia(ctx, difficulty: str = "easy"):
    if difficulty.lower() not in ["easy", "medium", "hard"]:
        await ctx.send("Please choose a difficulty level: easy, medium, or hard.")
        return

    async with aiohttp.ClientSession() as session:
        url = f"https://the-trivia-api.com/api/questions?limit=10&difficulty={difficulty.lower()}"
        async with session.get(url) as response:
            if response.status != 200:
                await ctx.send("Failed to fetch trivia questions.")
                return
            questions = await response.json()

    if not questions:
        await ctx.send("No trivia questions found for the specified difficulty.")
        return

    view = TriviaView(ctx, questions)
    for _ in range(4):
...
class WhatBeatsRockGame:
    def __init__(self, ctx):
        self.ctx = ctx
        self.current_object = "Rock"
        self.score = 0
        self.message = None
        self.model = ai.GenerativeModel("gemini-pro")  # Initialize the model once
    ...
    async def ask_question(self):
        embed = discord.Embed(
            title="What Beats Rock? (Single Player)",
            description=f"**What beats {self.current_object}?**",
            color=discord.Color.blue()
        )
        embed.set_footer(text="Type your answer in chat within 15 seconds.")
        self.message = await self.ctx.send(embed=embed)

        try:
            response = await self.get_user_response()
            await self.evaluate_response(response)
        except asyncio.TimeoutError:
            await self.end_game(f"⏳ Time's up! You reached **{self.score} points**.")
    ...
    async def get_user_response(self):
        ...
        try:
            return await self.ctx.bot.wait_for("message", check=check, timeout=15)
        except asyncio.TimeoutError:
            raise

    async def evaluate_response(self, response):
        answer = response.content.strip().capitalize()
         # Handle the "same object" case first
        if answer == self.current_object:
            ai_response = f"No way! {answer} fighting itself? That's like punching a mirror."
        else:
            ai_prompt = f"""
            You are playing a fun word-association game called 'What Beats Rock?'.  
            The game follows **logic-based** and **creative reasoning** to determine if one object can beat another.  
            Your job is to determine **if '{answer}' beats '{self.current_object}'** and provide a **funny, witty explanation**.

            Examples:
            - "Paper beats Rock! Because, well, it covers it up like a terrible secret!"
            - "Fire beats Paper! Paper burns like my motivation on a Monday."
            - "Water beats Fire! Because fire's worst enemy is a well-timed splash!"

            ### **RULES**
            1. Decide logically if '{answer}' **beats** '{self.current_object}'.  
            2. Always **be funny, witty, and engaging** in your explanation.  
            3. Start your response with 'Yes' or 'No', then provide your witty explanation.
            """
            ai_response = await self.ask_ai(ai_prompt)


        if ai_response.lower().startswith("yes"):
            self.score += 1
            self.current_object = answer
            await self.ctx.send(f"✅ **Correct!** `{answer}` beats `{self.current_object}`.\n**AI Explanation:** {ai_response}")
            await asyncio.sleep(2)
            await self.ask_question()
        else:
            await self.end_game(f"❌ **Game Over!** `{answer}` doesn't logically beat `{self.current_object}`.\n**AI Explanation:** {ai_response}\nFinal Score: **{self.score}**.")



    async def ask_ai(self, prompt):
        """Queries Google's Gemini AI API for a response with humor and logic."""

        try:
            response = await self.model.generate_content(
                prompt=prompt,  # Use generate_message for Gemini Pro
            ...
            print(f"Gemini API Error: {e}") # Keep this for debugging.
            return f"AI Error: {str(e)}" # Return a user-friendly error message

    async def end_game(self, reason):
        embed = discord.Embed(title="Game Over!", description=reason, color=discord.Color.red())
        await self.ctx.send(embed=embed)

# Command to start the game
@bot.command(aliases=["wbr"])
async def whatbeatsrock(ctx):
    """Starts a single-player 'What Beats Rock' game with AI judgment."""
    game = WhatBeatsRockGame(ctx)
    await game.start_game()


bot.run(os.environ["DISCORD_TOKEN_BETA"])